var fs = require('fs'),
	...
	mem_store = require(__dirname + '/mem_store').set,
	...
	REQ_OVERRIDE = 'process.storage = {}; process.storage.set = ' + mem_store + '; var old_require = require; require = ' + new_require,
	...
	writeLogs = function (toWrite, lines, fpath) {
		...
		if (typeof toWrite === 'object') {
			//Traverse
			lines = writeLogs(toWrite[0], lines, fpath);
		} else {
			//Write to appropriate line
			//Line number should ALWAYS exist as third element
			//console.log('TOWRITE!!! ' + toWrite[2]);
			var lineNum = (toWrite[2] || 0) + 1;
			//console.log(lineNum);
			if (!lines[lineNum])
				return lines;
			...
			lines[lineNum] = lines[lineNum].substring(0, semiColonIdx + 1) + 
											'storage.set('+toWrite[0] + ',' + toWrite[1] + ',' + toWrite[2] + ',' + fpath + ');' +
											lines[lineNum].substr(semiColonIdx);
		...
		async.auto({
			...
			modifyFile : ['checkSemiColon', 'getNodeModules', function (callback, cbres) {
				var data = cbres.readFile,
					...
					assignment_parser = function (a) {
						...
						switch (a.right.type) {
							...
							case 'Literal' :
								break;
							default :
								console.log('HERE!!!'+a.right.loc.start.line);
					...
					resultArr = body_parser(parsed.body);
					//console.log(resultArr);
					withLogs = writeLogs(JSON.parse('[' + JSON.stringify(resultArr) + ']'), data.split('\n'), filepath);

					//console.log('withLogs');
					//console.log(withLogs);
					//process.exit()
				callback(null, REQ_OVERRIDE+'\nprocess.storage.set(\'HELLO\', \'asdasdasd\', 1, \'asdas\');');
			}],
